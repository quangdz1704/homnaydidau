...
import { ChevronRight, Cloud, Images, MessageCircle, Plus, Search, Settings2, Sparkles, Star, Trash2, X } from "lucide-react";
  ...
  return (
    <section className="content-page me-page">
      ...
      <div className="settings-grid">
        ...
        <section className="settings-card custom-list"><div className="section-title"><div><span className="eyebrow">TỰ BIÊN TỰ DIỄN</span><h2>Kèo của tôi ✨</h2></div><Button variant="secondary" onClick={() => setCustomOpen(true)}><Plus size={17} /> Thêm kèo</Button></div><p>Ý hay của riêng bạn cũng được tham gia vòng quay.</p>{store.custom.length === 0 ? <div className="inline-empty">Chưa có kèo tự tạo. Thêm một chiếc xem sao?</div> : store.custom.map((activity) => <div className="custom-row" key={activity.id}><span>{activity.emoji}</span><div><strong>{activity.title}</strong><small>{modeLabels[activity.modes[0]]} · {activity.enabled ? "Đang trong Shuffle" : "Đã tắt"}</small></div><button onClick={() => void store.deleteCustom(activity.id)} aria-label={`Xoá ${activity.title}`}><Trash2 size={18} /></button></div>)}</section>
      ...
      {customOpen && <CustomActivityModal onClose={() => setCustomOpen(false)} />}
    </section>
  );
}

function CustomActivityModal({ onClose }: { onClose: () => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const activity: Activity = { id: `custom-${crypto.randomUUID?.() ?? Date.now()}`, kind: "activity", title: String(data.get("title")), description: String(data.get("description")), emoji: String(data.get("emoji") || "✨"), modes: [String(data.get("mode")) as Mode], moods: ["any"], category: String(data.get("category")) as Category, budget: { min: 0, max: maxBudget }, durationMinutes: { min: Math.min(30, maxDuration), max: maxDuration }, challenge: String(data.get("challenge") || "Cứ làm theo cách vui nhất."), source: "custom", enabled: data.get("enabled") === "on" };
    await addCustom(activity); onClose();
  }
  return <Modal label="Thêm kèo của bạn" onClose={onClose}><form className="custom-form" onSubmit={(event) => void submit(event)}><div className="modal-heading"><div><span className="eyebrow">TỰ BIÊN TỰ DIỄN</span><h2>Thêm kèo của bạn ✨</h2></div><button type="button" onClick={onClose} aria-label="Đóng"><X /></button></div><div className="form-row"><label className="field field--emoji"><span>Emoji</span><input name="emoji" defaultValue="✨" maxLength={3} /></label><label className="field"><span>Tên kèo</span><input name="title" placeholder="Ví dụ: Đi ăn ốc" required maxLength={60} /></label></div><label className="field"><span>Mô tả</span><textarea name="description" placeholder="Kèo này sẽ diễn ra thế nào?" required maxLength={220} /></label><div className="form-row"><label className="field"><span>Đi cùng ai?</span><select name="mode">{MODE_OPTIONS.map((item) => <option key={item.value} value={item.value}>{item.emoji} {item.label}</option>)}</select></label><label className="field"><span>Nhóm</span><select name="category">{Object.entries(CATEGORY_META).map(([id, item]) => <option key={id} value={id}>{item.emoji} {item.label}</option>)}</select></label></div><div className="form-row"><label className="field"><span>Budget tối đa (nghìn)</span><input name="budget" type="number" defaultValue={100} min={0} max={10000} /></label><label className="field"><span>Thời gian</span><select name="duration" defaultValue="120"><option value="60">1 tiếng</option><option value="120">2 tiếng</option><option value="180">3 tiếng</option><option value="360">Nửa ngày</option><option value="720">Cả ngày</option></select></label></div><label className="field"><span>Thử thách nhỏ</span><input name="challenge" placeholder="Thêm chút gia vị cho kèo..." /></label><label className="check-row"><input name="enabled" type="checkbox" defaultChecked /><span><strong>Cho kèo này vào Shuffle</strong><small>Tắt đi nếu chỉ muốn lưu làm ý tưởng.</small></span></label><Button type="submit">Lưu kèo này ✨</Button></form></Modal>;
}
